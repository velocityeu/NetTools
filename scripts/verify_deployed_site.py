...
def verify_deployment(url, expected, timeout=600, interval=15,
                      fetch=fetch_metadata, now=time.monotonic, wait=time.sleep):
    if url != SITE_URL:
    ...
    while now() < deadline:
        attempts += 1
        request_url = url + "release-data.json?verify=" + expected + "-" + str(attempts)
        try:
            data = fetch(request_url, min(30, max(0.1, deadline - now())))
            actual = hashlib.sha256(data).hexdigest()
            if actual == expected:
                print(f"Verified public release metadata SHA-256 {expected} after {attempts} request(s).")
                return
...
if __name__ == "__main__":
    ...
    verify_deployment(args.url, args.sha256, args.timeout, args.interval)
